    ...
    V: VMExecutor,
{
    ...
    pub fn execute_and_commit_chunk(
        ...
        for (txn_info_in_proof, (i, txn_data)) in itertools::zip_eq(
            ...
            let local_status = match txn_data.status() {
                TransactionStatus::Keep(s) => s.major_status,
                _ => panic!("impossible"),
            };
            ensure!(
                txn_info_in_proof.major_status() == local_status,
                "Status code does not match for {}-th transaction in chunk. Status in proof: {:?}. \
                 Local status: {:?}.",
                i,
                txn_info_in_proof.major_status(),
                local_status,
            );
        }
