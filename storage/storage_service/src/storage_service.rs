...
use types::transaction::{Version, TransactionListWithProof, TransactionToCommit};
use types::get_with_proof::{RequestItem, ResponseItem, UpdateToLatestLedgerResponse};
use types::ledger_info::LedgerInfoWithSignatures;
use crypto::ed25519::Ed25519Signature;
use types::validator_change::ValidatorChangeEventWithProof;
use std::pin::Pin;
...
use storage_proto::{StartupInfo, GetAccountStateWithProofByVersionResponse, GetLatestLedgerInfosPerEpochResponse};
use types::get_with_proof::UpdateToLatestLedgerRequest;
use proto_conv::{FromProto, IntoProto};
use std::sync::Arc;
use core::borrow::{Borrow, BorrowMut};
use storage_proto::proto::storage::{GetTransactionsRequest, GetAccountStateWithProofByVersionRequest, GetLatestLedgerInfosPerEpochRequest, SaveTransactionsRequest};
...
pub fn start_storage_service_and_return_service(config: &NodeConfig) -> (ServerHandle, Arc<StorageService>) {
...
impl StorageRead for StorageService {
    ...
    fn update_to_latest_ledger_async(
        ...
        Box<
            dyn Future<
                Output=Result<(Vec<ResponseItem>,
                               LedgerInfoWithSignatures<Ed25519Signature>,
                               Vec<ValidatorChangeEventWithProof<Ed25519Signature>>)>,
            > + Send,
        >,
    > {
        ...
        let resp = self.update_to_latest_ledger_inner(req.into_proto()).expect("update_to_latest_ledger_inner response err.");
        async {
    ...
    fn get_transactions(
        ...
        block_on(self.get_transactions_async(start_version, batch_size, ledger_version, fetch_events))
    }
    ...
    fn get_transactions_async(
        ...
        let mut resp = self.get_transactions_inner(req).expect("get_transactions_inner response err.");
        let proof = resp.take_txn_list_with_proof();
        async {
            TransactionListWithProof::from_proto(proof)
        }.boxed()
    }
    ...
    fn get_account_state_with_proof_by_version_async(
        ...
        let resp = self.get_account_state_with_proof_by_version_inner(req).expect("get_account_state_with_proof_by_version_inner response err.");
        async {
    ...
    fn get_startup_info_async(
        ...
        async {
            let tmp = match info {
                Some(i) => {
                    match StartupInfo::from_proto(i) {
                        Ok(data) => Some(data),
                        Err(e) => None
                    }
                }
    ...
    fn get_latest_ledger_infos_per_epoch_async(
        ...
        let resp = self.get_latest_ledger_infos_per_epoch_inner(req).expect("get_latest_ledger_infos_per_epoch_async response err.");
        async {
