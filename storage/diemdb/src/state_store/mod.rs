...
impl StateStore {
    ...
    pub fn put_account_state_sets<'a>(
        &self,
        account_state_sets: impl Iterator<
            Item = &'a HashMap<AccountAddress, (HashValue, AccountStateBlob)>,
        >,
        first_version: Version,
        cs: &mut ChangeSet,
    ) -> Result<Vec<HashValue>> {
        let blob_sets = account_state_sets.map(|account_states| {
            account_states
                .into_iter()
                .map(|(_, (hash, blob))| (*hash, blob.clone()))
        });
        ...
        tree_update_batch
            ...
            .iter()
            .try_for_each(|row| cs.batch.put::<StaleNodeIndexSchema>(row, &()))?;
