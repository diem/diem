...
mod test {
    ...
    fn verify_all_configs() {
        ...
        let _ = vec![
            "data/configs/single.node.config.toml",
        ]
        .iter()
        .map(|path| NodeConfig::load(PathBuf::from(path)).expect(&format!("Error in {}", path)))
        .collect::<Vec<_>>();
        ...
        .map(|path| {
            ...
            let mut contents = String::new();
            file.read_to_string(&mut contents).expect(&format!("Unable to read file: {}", path));
            ...
            NodeConfig::parse(&new_contents).expect(&format!("Error in {}", path));
        })
        .collect::<Vec<_>>();
    }
}
