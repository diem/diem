...
pub mod tests {
    ...
    fn test_set_operator() {
        ...
        // Owner uploads key to shared storage and initializes address in local storage
        let owner_key = storage_helper
            .owner_key(local_owner_ns, remote_owner_ns)
            .unwrap();
        ...
        let local_set_operator_tx = storage_helper
            .set_operator(operator_name, local_owner_ns, remote_owner_ns)
            .unwrap();

        // Verify that a transaction setting the operator was uploaded to the remote storage
        let remote_storage = storage_helper.storage(remote_owner_ns.into());
        let uploaded_set_operator_tx = remote_storage
            .get(constants::VALIDATOR_OPERATOR)
            .unwrap()
            .value
            .transaction()
            .unwrap();
        assert_eq!(local_set_operator_tx, uploaded_set_operator_tx);

        // Verify the transaction sender is the owner account address
        let owner_account = account_address::from_public_key(&owner_key);
        let uploaded_user_transaction = uploaded_set_operator_tx.as_signed_user_txn().unwrap();
        assert_eq!(owner_account, uploaded_user_transaction.sender());

        // Verify the chosen operator in the transaction has the correct account address
        match uploaded_user_transaction.payload() {
            TransactionPayload::Script(script) => {
                ...
                match script.args().get(0).unwrap() {
                    ...
                    _ => panic!(
                        "Found an invalid argument type for the set-operator transaction script!"
                    ),
                };
            }
            _ => panic!("Invalid set-operator transaction payload found!"),
        };
    }

    #[test]
    fn test_set_operator_and_validator_configs() {
        let storage_helper = StorageHelper::new();

        // Initialize storage for owner
        let local_owner_ns = "local_owner";
        let remote_owner_ns = "remote_owner";
        storage_helper.initialize(local_owner_ns.into());

        // Initialize storage for operator
        let local_operator_ns = "local_operator";
        let remote_operator_ns = "remote_operator";
        storage_helper.initialize(local_operator_ns.into());

        // Owner uploads key to shared storage and initializes address in local storage
        let _ = storage_helper
            .owner_key(local_owner_ns, remote_owner_ns)
            .unwrap();
        ...
        // Owner calls the set-operator command
        let local_set_operator_tx = storage_helper
            .set_operator(remote_operator_ns, local_owner_ns, remote_owner_ns)
            .unwrap();

        // Operator calls the validator-config command
        let local_validator_config_tx = storage_helper
            .validator_config(
                remote_owner_ns,
                "/ip4/0.0.0.0/tcp/6180".parse().unwrap(),
                "/ip4/0.0.0.0/tcp/6180".parse().unwrap(),
        ...
        // Verify that a transaction setting the operator was uploaded to the owner remote storage
        let remote_storage = storage_helper.storage(remote_owner_ns.into());
        let uploaded_set_operator_tx = remote_storage
            .get(constants::VALIDATOR_OPERATOR)
            .unwrap()
        ...
        // Verify that a validator config transaction was uploaded to the operator remote storage
        let remote_storage = storage_helper.storage(remote_operator_ns.into());
        let uploaded_validator_config_tx = remote_storage
            .get(constants::VALIDATOR_CONFIG)
            .unwrap()
            .value
            .transaction()
            .unwrap();
        assert_eq!(local_validator_config_tx, uploaded_validator_config_tx);
    }
