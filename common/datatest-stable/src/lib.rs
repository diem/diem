...
//! ```
//!
//! 2. Call the `libra_datatest_stable::harness!(testfn, root, pattern)` macro with the following
//! parameters:
//! * `testfn` - The test function to be executed on each matching input. This function must have
//!   the type `fn(&Path) -> libra_datatest_stable::Result<()>`
...
//! `libra_datatest_stable::harness!(testfn1, root1, pattern1, testfn2, root2, pattern2)`
