...
impl Experiment for CompatibilityTest {
    ...
    async fn run(&mut self, context: &mut Context<'_>) -> anyhow::Result<()> {
        ...
        if disable_fn_upgrade {
            ...
            info!("{}", msg);
            context.report.report_text(msg);
            context.cluster_builder.cluster_swarm.delete_all().await?;
            context.cluster_builder.current_tag = self.updated_image_tag.clone();
            context
                .cluster_builder
                .setup_cluster(context.cluster_builder_params)
                .await?;
            return Ok(());
        } else {
            let msg = format!(
            ...
            context.report.report_text(msg);
            update_batch_instance(context, &self.full_nodes, self.updated_image_tag.clone())
                .await?;
        }
