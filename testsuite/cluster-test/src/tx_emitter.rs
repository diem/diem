...
impl SubmissionWorker {
    ...
    async fn run(mut self) -> Vec<AccountData> {
        ...
        while !self.stop.load(Ordering::Relaxed) {
            ...
            let mut tx_latency_sum = 0u64;
            for request in requests {
                let cur_time = Instant::now();
                let wait_util = cur_time + wait;
                tx_latency_sum += (cur_time - start_time).as_millis() as u64;
            ...
            if self.params.wait_committed {
                    ...
                    wait_for_accounts_sequence(&self.client, &mut self.accounts).await
                {
                    ...
                    self.stats.latency.fetch_add(
                        ...
                        // Simplified from:
                        // end_time * num_committed - (tx_latency_sum/num_requests) * num_committed
                        (end_time - tx_latency_sum / num_requests as u64) * num_committed as u64,
                        Ordering::Relaxed,
                    );
                    info!(
                    ...
                    self.stats.latency.fetch_add(
                        end_time * num_requests as u64 - tx_latency_sum,
                        Ordering::Relaxed,
                    );
                }
            }
