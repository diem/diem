...
///////////////////////////////////
// Multiaddr Compatibiltiy Layer //
///////////////////////////////////

use parity_multiaddr::{Multiaddr, Protocol as MaProtocol};
use std::{borrow::Cow, convert::TryInto};

#[derive(Error, Debug)]
pub enum CompatError {
    #[error("unsupported multiaddr protocol: {0}")]
    UnsupportedMultiaddrProtocol(String),

    #[error("unsupported network address protocol: {0}")]
    UnsupportedNetworkProtocol(Protocol),

    #[error("{0}")]
    ParseError(#[from] ParseError),
}

impl TryFrom<Multiaddr> for NetworkAddress {
    type Error = CompatError;

    fn try_from(multiaddr: Multiaddr) -> Result<NetworkAddress, Self::Error> {
        let res_protos = multiaddr
            .into_iter()
            .map(Protocol::try_from)
            .collect::<Result<Vec<_>, CompatError>>();
        res_protos.map(NetworkAddress::new)
    }
}

impl TryInto<Multiaddr> for NetworkAddress {
    type Error = CompatError;

    fn try_into<'a>(self) -> Result<Multiaddr, Self::Error> {
        self.0
            .into_iter()
            .map(Protocol::try_into)
            .collect::<Result<Multiaddr, CompatError>>()
    }
}

impl<'a> TryFrom<MaProtocol<'a>> for Protocol {
    type Error = CompatError;

    fn try_from(proto: MaProtocol<'a>) -> Result<Protocol, Self::Error> {
        let proto = match proto {
            MaProtocol::Ip4(addr) => Protocol::Ip4(addr),
            MaProtocol::Ip6(addr) => Protocol::Ip6(addr),
            MaProtocol::Dns4(dnsname) => Protocol::Dns4(DnsName::try_from(dnsname.into_owned())?),
            MaProtocol::Dns6(dnsname) => Protocol::Dns6(DnsName::try_from(dnsname.into_owned())?),
            MaProtocol::Memory(port) => Protocol::Memory(port as u16),
            MaProtocol::Tcp(port) => Protocol::Tcp(port),
            unsupported => {
                let proto_string = format!("{}", unsupported);
                return Err(CompatError::UnsupportedMultiaddrProtocol(proto_string));
            }
        };
        Ok(proto)
    }
...
impl<'a> TryInto<MaProtocol<'a>> for Protocol {
    type Error = CompatError;

    fn try_into(self) -> Result<MaProtocol<'a>, Self::Error> {
        let proto = match self {
            Protocol::Ip4(addr) => MaProtocol::Ip4(addr),
            Protocol::Ip6(addr) => MaProtocol::Ip6(addr),
            Protocol::Dns4(addr) => MaProtocol::Dns4(Cow::Owned(addr.into())),
            Protocol::Dns6(addr) => MaProtocol::Dns6(Cow::Owned(addr.into())),
            Protocol::Memory(port) => MaProtocol::Memory(port as u64),
            Protocol::Tcp(port) => MaProtocol::Tcp(port),
            unsupported => return Err(CompatError::UnsupportedNetworkProtocol(unsupported)),
        };
        Ok(proto)
    }
}
