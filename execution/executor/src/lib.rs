    ...
    V: VMExecutor,
{
    ...
    fn get_executed_trees(&self, block_id: HashValue) -> Result<ExecutedTrees, Error> {
        let executed_trees = if block_id == self.cache.read().committed_block_id() {
            self.cache.read().committed_trees().clone()
        } else {
            self.cache
                .read()
                .get_block(&block_id)?
    ...
    fn get_executed_state_view<'a>(
        ...
        VerifiedStateView::new(
            id,
            Arc::clone(&self.db.reader),
            self.cache.read().committed_trees().version(),
            self.cache.read().committed_trees().state_root(),
            executed_trees.state_tree(),
        )
    }
...
impl<V: VMExecutor> ChunkExecutor for Executor<V> {
    fn execute_and_commit_chunk(
        ...
        if let Some(ledger_info_with_sigs) = &ledger_info_to_commit {
            write_lock.update_block_tree_root(
                output_trees,
...
impl<V: VMExecutor> BlockExecutor for Executor<V> {
    ...
    fn commit_blocks(
        ...
        if num_txns_in_li == num_persistent_txns {
            return Ok(read_lock.committed_txns_and_events());
        }
