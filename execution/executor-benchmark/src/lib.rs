...
fn create_storage_service_and_executor(
    ...
    let executor = Executor::new(
        db_rw, //StorageClient::new(&config.storage.address, config.storage.timeout_ms).into(),
    );
...
pub fn run_benchmark(
    ...
    let sender = exe_thread.join().unwrap();
    drop(sender);
