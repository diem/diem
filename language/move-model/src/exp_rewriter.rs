...
use crate::{
    ast::{Exp, LocalVarDecl, MemoryLabel, Operation, RcExp, TempIndex, Value},
...
pub struct ExpRewriter<'env, 'rewriter> {
    env: &'env GlobalEnv,
    replacer: &'rewriter mut dyn FnMut(NodeId, RewriteTarget) -> Option<RcExp>,
...
impl<'env, 'rewriter> ExpRewriter<'env, 'rewriter> {
    ...
    where
        F: FnMut(NodeId, RewriteTarget) -> Option<RcExp>,
    {
        ExpRewriter {
            env,
            replacer,
...
impl<'env, 'rewriter> ExpRewriterFunctions for ExpRewriter<'env, 'rewriter> {
    ...
    fn rewrite_node_id(&mut self, id: NodeId) -> Option<NodeId> {
        Exp::instantiate_node(self.env, id, self.type_args)
    }
}
...
pub trait ExpRewriterFunctions {
    ...
    fn rewrite_vec(&mut self, exps: &[RcExp]) -> Vec<RcExp> {
    ...
    fn rewrite_value(&mut self, id: NodeId, value: &Value) -> Option<RcExp> {
        None
    }
    fn rewrite_spec_var(
        &mut self,
    ...
    fn rewrite_invoke(&mut self, id: NodeId, target: &RcExp, args: &[RcExp]) -> Option<RcExp> {
        None
    }
    fn rewrite_lambda(&mut self, id: NodeId, vars: &[LocalVarDecl], body: &RcExp) -> Option<RcExp> {
        None
    }
    fn rewrite_block(&mut self, id: NodeId, vars: &[LocalVarDecl], body: &RcExp) -> Option<RcExp> {
        None
    }
    fn rewrite_quant(
        &mut self,
        id: NodeId,
        vars: &[(LocalVarDecl, RcExp)],
        triggers: &[Vec<RcExp>],
        cond: &Option<RcExp>,
        body: &RcExp,
    ) -> Option<RcExp> {
    ...
    fn rewrite_exp_descent(&mut self, exp: RcExp) -> RcExp {
        use Exp::*;
        match exp.as_ref() {
            Value(id, value) => {
                ...
                if let Some(new_exp) = self.rewrite_value(new_id, value) {
                    ...
                    Rc::new(Value(new_id, value.clone()))
                } else {
                    exp
                }
            }
            ...
            SpecVar(id, mid, vid, label) => {
                ...
                if let Some(new_exp) = self.rewrite_spec_var(new_id, *mid, *vid, label) {
                    ...
                    Rc::new(SpecVar(new_id, *mid, *vid, label.to_owned()))
                } else {
                    exp
                }
            }
            Call(id, oper, args) => {
                ...
                if let Some(new_exp) = self.rewrite_call(new_id, oper, &args_ref) {
                    ...
                    Rc::new(Call(new_id, oper.clone(), args_owned))
                } else {
                    exp
                }
            }
            Invoke(id, target, args) => {
                ...
                if let Some(new_exp) = self.rewrite_invoke(new_id, &new_target, args_ref) {
                    ...
                    Rc::new(Invoke(new_id, new_target, args_owned))
                } else {
                    exp
                }
            }
            Lambda(id, vars, body) => {
                ...
                if let Some(new_exp) = self.rewrite_lambda(new_id, &new_vars, &new_body) {
                    ...
                    Rc::new(Lambda(new_id, new_vars, new_body))
                } else {
                    exp
                }
            }
            Block(id, vars, body) => {
                ...
                if let Some(new_exp) = self.rewrite_block(new_id, &new_vars, &new_body) {
                    ...
                    Rc::new(Block(new_id, new_vars, new_body))
                } else {
                    exp
                }
            }
            ...
            IfElse(id, cond, then, else_) => {
                ...
                if let Some(new_exp) = self.rewrite_if_else(new_id, &new_cond, &new_then, &new_else)
                {
                    ...
                    Rc::new(IfElse(new_id, new_cond, new_then, new_else))
                } else {
                    exp
                }
            }
    ...
    fn internal_rewrite_exp(&mut self, exp: &RcExp) -> (bool, RcExp) {
        let new_exp = self.rewrite_exp(exp.clone());
        (!Rc::ptr_eq(exp, &new_exp), new_exp)
    }

    fn internal_rewrite_vec(&mut self, exps: &[RcExp]) -> Option<Vec<RcExp>> {
    ...
    fn internal_rewrite_quant_decls(
        &mut self,
        decls: &[(LocalVarDecl, RcExp)],
    ) -> (bool, Vec<(LocalVarDecl, RcExp)>) {
        let mut change = false;
        let new_decls = decls
            .iter()
            .map(|(d, e)| {
