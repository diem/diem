...
use crate::{
    ...
    ADD_TO_SCRIPT_ALLOW_LIST_BYTES, ADD_VALIDATOR_AND_RECONFIGURE_BYTES, BURN_BYTES,
    BURN_TXN_FEES_BYTES, CANCEL_BURN_BYTES, CREATE_CHILD_VASP_ACCOUNT_BYTES,
    CREATE_DESIGNATED_DEALER_BYTES, CREATE_PARENT_VASP_ACCOUNT_BYTES,
    CREATE_RECOVERY_ADDRESS_BYTES, CREATE_VALIDATOR_ACCOUNT_BYTES,
    CREATE_VALIDATOR_OPERATOR_ACCOUNT_BYTES, FREEZE_ACCOUNT_BYTES,
...
fn test_byte_patterns() {
    ...
    for script in LegacyStdlibScript::all() {
        let bytes_from_pattern = match script {
            ...
            S::AddScriptAllowList => ADD_TO_SCRIPT_ALLOW_LIST_BYTES!().to_vec(),
