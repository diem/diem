...
pub fn native_ed25519_signature_verification(
    ...
    let pk = match ed25519::VerifyingKey::try_from(pubkey.as_slice()) {
        Ok(pk) => pk,
        Err(_) => {
            return Ok(NativeResult::err(
                cost,
...
) -> Vec<(ed25519::VerifyingKey, ed25519::Signature)> {
    let mut sig_index = 0;
    let mut keys_and_signatures: Vec<(ed25519::VerifyingKey, ed25519::Signature)> =
