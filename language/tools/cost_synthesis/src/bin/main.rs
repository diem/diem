...
fn stack_instructions() {
    ...
    let stack_opcodes: Vec<Bytecode> = vec![
        ...
        ReleaseRef,
        FreezeRef,
        MoveToSender(StructDefinitionIndex::new(0), vec![]),
        Exists(StructDefinitionIndex::new(0), vec![]),
        BorrowGlobal(StructDefinitionIndex::new(0), vec![]),
        MoveFrom(StructDefinitionIndex::new(0), vec![]),
        BorrowField(FieldDefinitionIndex::new(0)),
        CopyLoc(0),
        MoveLoc(0),
        BorrowLoc(0),
        StLoc(0),
        Unpack(StructDefinitionIndex::new(0), vec![]),
        Pack(StructDefinitionIndex::new(0), vec![]),
        Call(FunctionHandleIndex::new(0), vec![]),
        CreateAccount,
        Sub,
        Ret,
        Add,
