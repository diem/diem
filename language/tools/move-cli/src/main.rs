...
use move_cli::{OnDiskStateView, MOVE_DATA, MOVE_SRC, MOVE_SRC_ADDRESS};
use move_core_types::{
...
fn compile(
    ...
    let move_src = Path::new(&args.move_src);
    ...
    let mut user_lib_files: Vec<String> = if move_src.exists() {
        fs::read_dir(move_src)?
    ...
    let has_user_modules = !user_lib_files.is_empty();

    if has_user_modules {
        print!("Compiling {:?} module(s)", user_lib_files.len());
    }
