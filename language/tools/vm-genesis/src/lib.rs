...
pub fn encode_genesis_transaction(
    ...
    Transaction::GenesisTransaction(WriteSetPayload::Direct(encode_genesis_change_set(
        ...
        stdlib_modules(StdLibOptions::Compiled).0.unwrap(), // Must use compiled stdlib,
...
macro_rules! stdlib_modules_bytes {
    ($stdlib_options: expr) => {{
        match stdlib_modules($stdlib_options) {
...
pub fn generate_genesis_change_set_for_testing(stdlib_options: StdLibOptions) -> ChangeSet {
    generate_test_genesis(
        &stdlib_modules_bytes!(stdlib_options),
...
pub fn test_genesis_change_set_and_validators(count: Option<usize>) -> (ChangeSet, Vec<Validator>) {
    generate_test_genesis(
        &stdlib_modules_bytes!(StdLibOptions::Compiled),
