...
module LibraAccount {
    ...
    public deposit(payee: address, to_deposit: LibraCoin.T) acquires T {
        ...
        borrow_global_mut<T>(payer).sent_events.emit_event(SentPaymentEvent { amount, payee });

        let payee_account = borrow_global_mut<T>(payee);
        // Deposit the `to_deposit` coin
        payee_account.balance.deposit(to_deposit);
        // Log a received event
        payee_account.received_events.emit_event(ReceivedPaymentEvent { amount, payer })
    }
    ...
    public withdraw_from_sender(amount: u64): LibraCoin.T acquires T {
        ...
        sender_account.balance.withdraw(amount)
    }
    ...
    public withdraw_with_capability(
        ...
        acquires T
    {
        borrow_global_mut<T>(cap.account_address).balance.withdraw(amount)
    }
    ...
    public balance(addr: address): u64 acquires T {
        borrow_global<T>(addr).balance.value()
    }
    ...
    prologue() acquires T {
        ...
        Transaction.assert(sender_account.balance.value() >= max_transaction_fee, 6);
    ...
    epilogue() acquires T {
        ...
        Transaction.assert(sender_account.balance.value() >= transaction_fee_amount, 6);
        let transaction_fee = sender_account.balance.withdraw(transaction_fee_amount);
        ...
        borrow_global_mut<T>(0xFEE).balance.deposit(transaction_fee)
    }
