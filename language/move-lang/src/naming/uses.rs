...
struct Context<'a> {
    ...
    // - if A uses B, A deps on B
    // - if A friends B, B deps on A
...
impl<'a> Context<'a> {
    ...
    fn add_neighbor(&mut self, mident: ModuleIdent, dep_type: DepType, loc: Loc) {
        ...
        let (upstream, downstream) = match dep_type {
...
fn cycle_error(
    ...
    let mut cycle_strings: String = cycle
        ...
        .map(|pair| {
            ...
            let relations = deps.get(upstream).unwrap().get(downstream).unwrap();
            ...
            format!("'{}' {} ", upstream, verb)
        })
        .collect();
    cycle_strings.push_str(&format!("'{}'", cycle.last().unwrap()));

    // For printing uses, sort the cycle by location (earliest first)
    let (dep_type, cycle_loc, member, host) = best_cycle_loc(deps, cycle);

    let use_msg = format!(
        "Invalid {} of module '{}' in module '{}'.",
        match dep_type {
            DepType::Use => "use",
    ...
    let cycle_msg = format!(
        "{} this module creates a dependency cycle: {}",
        match dep_type {
            DepType::Use => "Using",
            DepType::Friend => "Friending",
        },
        cycle_strings
    );
...
fn best_cycle_loc<'a>(
    ...
    assert!(len >= 3);
    let first = cycle[0];
    let upstream = cycle[len.wrapping_sub(2)];
    let downstream = cycle[len.wrapping_sub(1)];
    assert_eq!(first, downstream);
    let cycle_locs = deps.get(upstream).unwrap().get(downstream).unwrap();
    if let Some(loc) = cycle_locs.get(&DepType::Use) {
        (DepType::Use, *loc, downstream, upstream)
    } else {
        let loc = cycle_locs.get(&DepType::Friend).unwrap();
        (DepType::Friend, *loc, upstream, downstream)
    }
}
