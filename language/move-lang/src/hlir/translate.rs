...
fn module(
    ...
    let structs = t_structs.map(|name, s| struct_def(context, name, s));
    context.add_struct_fields(&structs);

    let constants = t_constants.map(|name, c| constant(context, name, c));
    let functions = t_functions.map(|name, f| function(context, name, f));
