...
module SharedKeyRotation {
    ...
    public fun publish(account: &signer, cap: LibraAccount::KeyRotationCapability, master_key_address: address) {
    ...
    public fun rotate(account: &signer, wallet_address: address, new_key: vector<u8>) acquires T {
        ...
        let cap_addr = *LibraAccount::key_rotation_capability_address(&wallet_ref.cap);
        assert((wallet_ref.master_key_address == sender) || (cap_addr == sender), 77);
        LibraAccount::rotate_authentication_key(&wallet_ref.cap, new_key);
    }
}
...
fun main(account: &signer) {
    ...
    SharedKeyRotation::publish(account, LibraAccount::extract_key_rotation_capability(account), {{bob}});
}
}
...
fun main(account: &signer) {
    ...
    // details to be found in LibraAccount::make_account
