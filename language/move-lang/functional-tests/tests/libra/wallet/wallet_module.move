...
module ColdWallet {
    ...
    struct Transaction {
        ...
        payee: address,
        // The amount of Libra<Coin1> sent
        amount: u64
    }
    ...
    // Libra account, without assuming there's a cold wallet module
    // under that account
    public fun withdraw_from_payer(
        ...
        let account_balance = Libra::value(&payer_ref.balance);
        ...
        if (has_authority) {
            ...
            let withdraw_amount = Libra::withdraw(&mut payer_ref.balance, amount);
            // LibraAccount no longer has this API
            //LibraAccount::deposit(payer_, payee, withdraw_amount);
            Libra::destroy_zero(withdraw_amount);
        };
    }
}
