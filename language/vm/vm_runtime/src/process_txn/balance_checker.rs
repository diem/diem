...
use types::language_storage::{ModuleId, StructTag};
use types::vm_error::{StatusCode, VMStatus};
use types::write_set::{WriteOp, WriteSet};
use vm::{
    access::ModuleAccess,
    file_format::{
        SignatureToken, StructFieldInformation,
    },
    views::StructHandleView,
};
use vm::errors::VMResult;
use vm::gas_schedule::{GasAlgebra, GasUnits};
use vm_runtime_types::loaded_data::struct_def::StructDef;
use vm_runtime_types::value::{Struct, Value};

use crate::code_cache::module_cache::ModuleCache;
...
impl<'alloc, 'txn> StructFieldScanner<'alloc, 'txn> {
    ...
    pub fn scan(&self, tag: &StructTag, value: &Struct, predicate: &dyn Fn(&StructTag, &Struct) -> bool) -> VMResult<Vec<(StructTag, Struct)>> {
    ...
    fn load_module_by_tag(&self, tag: &StructTag) -> VMResult<&LoadedModule> {
        self.module_cache.get_loaded_module(&ModuleId::new(tag.address, tag.module.clone()))?.ok_or(VMStatus::new(StatusCode::MISSING_DATA))
    }

    fn scan_struct_feild(&self, tag: &StructTag, value: &Struct, resources: &mut Vec<(StructTag, Struct)>, predicate: &dyn Fn(&StructTag, &Struct) -> bool) -> VMResult<()> {
        let module = self.load_module_by_tag(tag)?;
        let struct_def_idx = module
            .struct_defs_table
            .get(&tag.name).ok_or(VMStatus::new(StatusCode::LINKER_ERROR))?;
        let struct_def = module.struct_def_at(*struct_def_idx);
        //let struct_handle = module.struct_handle_at(struct_def.struct_handle);
        match &struct_def.field_information {
            StructFieldInformation::Declared { field_count, fields } => {
                for (idx, field) in module.field_def_range(*field_count, *fields).iter().enumerate() {
                    ...
                    match field_signature {
                        SignatureToken::Struct(idx, _) => {
                            ...
                            let field_type_module_id = StructHandleView::new(module, struct_handle).module_id();
                            let field_module = self.module_cache.get_loaded_module(&field_type_module_id)?.ok_or(VMStatus::new(StatusCode::MISSING_DATA))?;
                            //let field_off_set = module.get_field_offset(*fields + idx)?;
                            //let value = value.get_field_value(field_off_set as usize)?;
                            let struct_value: Struct = field_value.value_as().expect("value must be struct");
                            ...
                            if predicate(&field_struct_tag, &struct_value) {
                                ...
                                self.scan_struct_feild(&field_struct_tag, &struct_value, resources, predicate)?;
                            }
                        }
                        _ => {
                            //continue
...
impl AssetCollector {
    ...
    pub fn asset_balance(value: &Struct) -> VMResult<u64> {
        let balance = value.get_field_value(0)?.value_as().expect("asset first field must be balance.");
        Ok(balance)
    }

    pub fn collect(&self, scanner: &StructFieldScanner, tag: &StructTag, resource: &Struct) -> VMResult<()> {
...
impl<'alloc, 'txn> BalanceChecker<'alloc, 'txn> {
    pub fn new(data_cache: &'txn dyn RemoteCache, module_cache: &'txn dyn ModuleCache<'alloc>) -> Self {
        Self {
            data_cache,
    ...
    fn get_resource(&self, ap: &AccessPath) -> VMResult<Option<(StructTag, Struct)>> {
        let tag = ap.resource_tag().expect("this access_path must contains resource tag.");
    ...
    fn deserialize_struct(&self, tag: &StructTag, value: &[u8]) -> VMResult<Struct> {
        ...
        let struct_value = Value::simple_deserialize(value, def)?.value_as().expect("value must be struct");
        Ok(struct_value)
    }
    ...
    fn resolve(&self, tag: &StructTag) -> VMResult<StructDef> {
        ...
        let struct_def_idx = module
            .struct_defs_table
            .get(&tag.name).ok_or(VMStatus::new(StatusCode::LINKER_ERROR))?;
        self.scanner.module_cache().resolve_struct_def(module, *struct_def_idx, &gas)?.ok_or(VMStatus::new(StatusCode::MISSING_DATA))
    }
    ...
    pub fn check_balance(&self, write_set: &WriteSet) -> Result<(), VMStatus> {
            ...
            match op {
                WriteOp::Deletion => {
                    if !access_path.is_code() {
                        old_resources.push(self.get_resource(access_path)?.ok_or(VMStatus::new(StatusCode::MISSING_DATA))?);
                    }
                }
                WriteOp::Value(value) => {
                    if !access_path.is_code() {
                        let tag = access_path.resource_tag().expect("this access_path must contains resource tag.");
        ...
        if old_assets != new_assets {
            ...
            err.set_message(format!("old assets {:?} and new assets {:?} is not equals.", old_assets, new_assets));
            return Err(err);
        }
        Ok(())
    }
