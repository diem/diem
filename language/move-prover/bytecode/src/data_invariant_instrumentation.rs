...
use move_model::{
    ast,
    ast::{ConditionKind, Exp, QuantKind, RcExp, TempIndex},
...
impl<'a> Instrumenter<'a> {
    ...
    fn instrument_bytecode(&mut self, bc: Bytecode) {
        ...
        match bc {
            ...
            Prop(id, PropKind::Assume, exp) => {
                let mut rewriter = |e: RcExp| {
                    if let Exp::Call(_, ast::Operation::WellFormed, args) = e.as_ref() {
                ...
                let exp = Exp::rewrite(exp, &mut rewriter);
    ...
    fn translate_invariant(&self, deep: bool, value: RcExp) -> Vec<(Loc, RcExp)> {
    ...
    fn translate_invariant_for_struct(
        ...
        for cond in struct_env.get_spec().filter_kind(ConditionKind::Invariant) {
            ...
            let exp_rewriter = &mut |e: RcExp| match e.as_ref() {
                Call(id, oper @ Select(..), args) if args.is_empty() => {
                    Ok(Call(*id, oper.to_owned(), vec![value.clone()]).into_rc())
                }
                _ => Err(e),
            };
            // Also instantiate types.
            let env = self.builder.global_env();
            let node_rewriter = &mut |id: NodeId| Exp::instantiate_node(env, id, targs);

            let exp = Exp::rewrite_exp_and_node_id(cond.exp.clone(), exp_rewriter, node_rewriter);
