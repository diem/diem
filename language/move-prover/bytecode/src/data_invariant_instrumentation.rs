...
//
// This transformation adds the data invariant to all occurrences of the `WellFormed` call
// which has been introduced by the spec instrumenter. It also handles PackRef/PackRefDeep
// instructions introduced by memory instrumentation, as well as the Pack instructions.
// The WellFormed expressions are maintained in the output for processing by the backend,
// in case type assumptions needed to be added by the backend (which depends on the compilation
// scheme.
