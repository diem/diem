...
module DiemTest {
    ...
    struct RegisterNewCurrency has store {}
    ...
    struct MintCapability<CoinType> has key { }
    ...
    struct BurnCapability<CoinType> has key { }
    ...
    struct Preburn<CoinType> has key {
    ...
    public fun preburn_with_resource<CoinType: store>(
        coin: Diem<CoinType>,
    ...
    public fun publish_preburn_to_account<CoinType: store>(
        account: &signer,
        tc_account: &signer
    ) acquires CurrencyInfo {
    ...
    /// resource under `preburn_address`, or, if the preburn to_burn area for
    /// `CoinType` is empty (error code 7).
    public fun burn_with_resource_cap<CoinType: store>(
        preburn: &mut Preburn<CoinType>,
        preburn_address: address,
    ...
    public fun join<CoinType: store>(xus: Diem<CoinType>, coin2: Diem<CoinType>): Diem<CoinType>  {
    ...
    fun assert_is_currency<CoinType: store>() {
        assert(is_currency<CoinType>(), ENOT_A_REGISTERED_CURRENCY);
    }

    fun assert_is_SCS_currency<CoinType: store>() acquires CurrencyInfo {
