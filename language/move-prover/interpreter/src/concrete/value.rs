...
impl TypedValue {
    ...
    /// Unwrap the pointer from the mutable reference to its original pointer
    pub fn replace_base_value(self, other: TypedValue) -> TypedValue {
        let (ty, _, ptr) = self.decompose();
        let (ty_other, val, _) = other.decompose();
        if cfg!(debug_assertions) {
            assert_eq!(ty, ty_other);
        }
        TypedValue { ty, val, ptr }
    }
