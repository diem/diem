...
use crate::concrete::error::{ErrorReason, InvariantViolation};
use crate::{
    concrete::{
        player,
        ty::{
...
fn check_and_convert_type_args_and_args(
    ...
    for (i, (arg, param)) in args.iter().zip(params.into_iter()).enumerate() {
        ...
        #[cfg(debug_assertions)]
        if local_ty != param.1 {
            return Err(err_type_mismatch_entrypoint_param_and_local(
                fun_env, i, &param.1, &local_ty,
            )
...
#[cfg(debug_assertions)]
fn err_type_mismatch_entrypoint_param_and_local(
    fun_env: &FunctionEnv,
    idx: usize,
    param_ty: &MT::Type,
    local_ty: &MT::Type,
) -> InvariantViolation {
    let ctxt = fun_env.get_type_display_ctxt();
    let message = format!(
        "param and local type mismatches for entrypoint function {} at index {}: \
             param type is {}, local type is {}",
        fun_env.get_full_name_str(),
        idx,
        param_ty.display(&ctxt),
        local_ty.display(&ctxt)
    );
    InvariantViolation::new(ErrorReason::TYPE_MISMATCH_ENTRYPOINT_PARAM_AND_LOCAL)
        .add_message(message)
}
