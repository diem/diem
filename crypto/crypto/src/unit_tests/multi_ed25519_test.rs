...
fn test_successful_public_key_serialization(
    ...
    assert_eq!(serialized.len(), n * ed25519::PUBLIC_KEY_LENGTH + 1);
    let reserialized = multi_ed25519::VerifyingKeys::try_from(&serialized[..]);
    assert!(reserialized.is_ok());
    assert_eq!(public_key, reserialized.unwrap());
}

fn test_failed_public_key_serialization(
    result: std::result::Result<multi_ed25519::VerifyingKeys, CryptoMaterialError>,
...
fn test_successful_signature_serialization(private_keys: &[ed25519::SigningKey], threshold: u8) {
    ...
    let multi_signature_serialized =
        multi_ed25519::MultiSignature::try_from(&multi_signature.to_bytes()[..]);
...
fn test_multi_ed25519_public_key_serialization() {
    ...
    let multi_key_31_bytes =
        multi_ed25519::VerifyingKeys::try_from(&[0u8; ed25519::PUBLIC_KEY_LENGTH - 1][..]);
    test_failed_public_key_serialization(multi_key_31_bytes, WrongLengthError);

    // Test try_from 32 bytes (should fail) because we always need ed25519::PUBLIC_KEY_LENGTH * N + 1
    // bytes (thus 32N + 1).
    let multi_key_32_bytes =
        multi_ed25519::VerifyingKeys::try_from(&[0u8; ed25519::PUBLIC_KEY_LENGTH][..]);
    ...
    let multi_key_34_bytes =
        multi_ed25519::VerifyingKeys::try_from(&[0u8; ed25519::PUBLIC_KEY_LENGTH + 2][..]);
    ...
    let multi_private_key_7of10 = multi_ed25519::SigningKeys::new(priv_keys_10, 7).unwrap();
    let multi_public_key_7of10 = multi_ed25519::VerifyingKeys::new(pub_keys_10, 7).unwrap();

    // Check that multi_ed25519::VerifyingKeys::from multi_ed25519::SigningKeys works as expected.
    let multi_public_key_7of10_from_multi_private_key =
        multi_ed25519::VerifyingKeys::from(&multi_private_key_7of10);
    assert_eq!(
...
fn test_publickey_smallorder() {
    ...
    let torsion_key = multi_ed25519::VerifyingKeys::try_from(&torsion_point_with_threshold_1[..]);
...
fn test_multi_ed25519_signature_serialization() {
    ...
    let multi_priv_key_1of3 = multi_ed25519::SigningKeys::new(priv_keys_3.to_vec(), 1).unwrap();
    let multi_pub_key_1of3 = multi_ed25519::VerifyingKeys::from(&multi_priv_key_1of3);
    assert!(multi_signature
    ...
    let multi_sig32 = multi_ed25519::MultiSignature::new(sig32_tuple);
    ...
    let multi_pub_key_32 = multi_ed25519::VerifyingKeys::new(pub_key_32, 32).unwrap();
    assert!(multi_sig32_unwrapped
        .verify(&MESSAGE_HASH, &multi_pub_key_32)
        .is_ok());

    // Fail to construct a multi_ed25519::MultiSignature object from 33 or more single signatures.
    ...
    let multi_sig33 = multi_ed25519::MultiSignature::new(sig33_tuple);
    ...
    // Fail to construct a multi_ed25519::MultiSignature object if there are duplicated indexes.
    ...
    let multi_sig3 = multi_ed25519::MultiSignature::new(sig3_tuple);
    ...
    // Fail to construct a multi_ed25519::MultiSignature object if an index is out of range.
...
fn test_multi_ed25519_signature_verification() {
    ...
    let multi_private_key_7of10 = multi_ed25519::SigningKeys::new(priv_keys_10.clone(), 7).unwrap();
    let multi_public_key_7of10 = multi_ed25519::VerifyingKeys::from(&multi_private_key_7of10);
    ...
    let multi_public_key_8of10 = multi_ed25519::VerifyingKeys::new(pub_keys_10.clone(), 8).unwrap();
    assert!(multi_signature_7of10
    ...
    let multi_public_key_6of10 = multi_ed25519::VerifyingKeys::new(pub_keys_10.clone(), 6).unwrap();
    assert!(multi_signature_7of10
        .verify(&MESSAGE_HASH, &multi_public_key_6of10)
        .is_ok());

    // Verifying a 7-of-10 signature against a reordered multi_ed25519::VerifyingKeys should fail.
    ...
    let multi_public_key_7of10_reversed =
        multi_ed25519::VerifyingKeys::new(pub_keys_10_reversed, 7).unwrap();
    assert!(multi_signature_7of10
    ...
    let multi_private_key_1of3 = multi_ed25519::SigningKeys::new(priv_keys_3.clone(), 1).unwrap();
    let multi_public_key_1of3 = multi_ed25519::VerifyingKeys::from(&multi_private_key_1of3);
    ...
    let multi_sig_signed_by_2nd_key =
        multi_ed25519::MultiSignature::new(vec![(sig_with_2nd_key, 1)]);
    ...
    let multi_sig_signed_by_2nd_key_wrong_index =
        multi_ed25519::MultiSignature::new(vec![(sig_with_2nd_key.clone(), 2)]);
    ...
    let multi_sig_signed_by_2nd_and_3rd_key = multi_ed25519::MultiSignature::new(vec![
    ...
    let multi_sig_signed_by_2nd_and_3rd_key_swapped = multi_ed25519::MultiSignature::new(vec![
    ...
    let multi_sig_signed_by_2nd_and_unrelated_key = multi_ed25519::MultiSignature::new(vec![
        (sig_with_2nd_key.clone(), 1),
    ...
    let multi_private_key_2of3 = multi_ed25519::SigningKeys::new(priv_keys_3.clone(), 2).unwrap();
    let multi_public_key_2of3 = multi_ed25519::VerifyingKeys::from(&multi_private_key_2of3);

    let sig_with_1st_key = priv_keys_3[0].sign_message(&MESSAGE_HASH);

    // Signing with the 1st and 2nd keys must succeed.
    let signed_by_1st_and_2nd_key = multi_ed25519::MultiSignature::new(vec![
    ...
    let signed_by_1st_and_3rd_key = multi_ed25519::MultiSignature::new(vec![
        (sig_with_1st_key.clone(), 0),
        (sig_with_3rd_key.clone(), 2),
    ]);
    ...
    let signed_by_2nd_and_3rd_key = multi_ed25519::MultiSignature::new(vec![
        (sig_with_2nd_key.clone(), 1),
        (sig_with_3rd_key.clone(), 2),
    ]);
    ...
    // Signing with the 2nd and 3rd keys must succeed.
    let signed_by_all_3_keys = multi_ed25519::MultiSignature::new(vec![
        (sig_with_1st_key, 0),
        (sig_with_2nd_key.clone(), 1),
        (sig_with_3rd_key, 2),
    ]);
    ...
    let signed_by_2nd_key = multi_ed25519::MultiSignature::new(vec![(sig_with_2nd_key, 1)]);
