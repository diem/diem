...
impl Cuckoo {
    ...
    pub fn verify(&self, header: &[u8], nonce: u32, proof: Vec<u64>) -> bool {
        if proof.len() != self.cycle_length {
            return false;
        }

        if proof[self.cycle_length - 1] > self.edge_mask {
            return false;
        }
        ...
        input.truncate(header.len() - 4); // drop last 4 bytes (u32) off the end
        input.write_u32::<LittleEndian>(nonce);
...
impl CuckooSip {
    ...
    pub fn edge(&self, val: u64, edge_mask: u64) -> (u64, u64) {
        let upper = self.hash(val << 1) & edge_mask;
        let lower = self.hash((val << 1) + 1) & edge_mask;

        (upper, lower)
    }

    pub fn input_to_keys(input: Vec<u8>) -> [u64; 4] {
        [
...
mod test {
    ...
    fn test_mine() {
        ...
        let input_hash = blake2b(32, &[], &input).as_bytes();
