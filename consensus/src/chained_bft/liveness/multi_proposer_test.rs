...
use crate::chained_bft::{
    ...
    liveness::{multi_proposer_election::MultiProposer, proposer_election::ProposerElection},
};
use crypto::ed25519::*;
use std::sync::Arc;
...
fn test_multi_proposer() {
    ...
    let pe: Arc<dyn ProposerElection<u32>> = Arc::new(MultiProposer::new(proposers.clone(), 2));
    let round = 1;
    let first_hash = MultiProposer::hash(round);
    let primary_idx = (first_hash % 8) as usize;
    let second_hash = MultiProposer::hash(first_hash);
...
fn test_multi_proposer_take_all() {
    ...
    let pe: Arc<dyn ProposerElection<u32>> = Arc::new(MultiProposer::new(proposers.clone(), 8));
...
fn test_multi_proposer_hash() {
    ...
    for round in 0..10000 {
        let idx = (MultiProposer::hash(round) % counts.len() as u64) as usize;
        counts[idx] += 1;
    }
    for c in counts {
        assert!(c > 900);
