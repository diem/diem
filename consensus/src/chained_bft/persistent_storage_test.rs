...
use crate::chained_bft::test_utils::{TestPayload, placeholder_ledger_info};
...
fn is_correct_consensus_root(blocks: &Vec<Block<TestPayload>>, b_idx_end: usize, found_root: (Block<TestPayload>, QuorumCert, QuorumCert)) -> bool {
...
/// Verify that the found root matches the expected root.
fn is_correct_storage_root(blocks: &Vec<Block<TestPayload>>, committed_b_idx: usize, found_root: (Block<TestPayload>, QuorumCert, QuorumCert)) -> bool {
...
fn is_genesis_root(found_root: (Block<TestPayload>, QuorumCert, QuorumCert)) -> bool {
    ...
    genesis == found_root.0 && QuorumCert::certificate_for_genesis() == found_root.1 && QuorumCert::certificate_for_genesis() == found_root.2
}
...
fn test_chain_restartability_root_from_consensusdb() {
    ...
    for b_idx_start in 2..blocks.len()-3 {
        for b_idx_end in b_idx_start+3..blocks.len() {
            ...
            let mut blocks_advanced = blocks[b_idx_start..b_idx_end+1].iter().cloned().collect();
            let mut qc_advanced = quorum_certs[b_idx_start..b_idx_end+1].iter().cloned().collect();
    ...
    for committed_b_idx in 2..blocks.len()-4 {
        ...
        storage_ledger.set_consensus_block_id(li_storage.id());
            ...
            for b_idx_end in committed_b_idx+4..blocks.len() {
                let mut blocks_advanced = blocks[committed_b_idx..b_idx_end+1].iter().cloned().collect();
                let mut qc_advanced = quorum_certs[committed_b_idx..b_idx_end+1].iter().cloned().collect();
                ...
                assert!(is_correct_storage_root(&blocks, committed_b_idx, found_root));
            }
    }
}
...
fn test_not_restartable_too_short() {
    ...
    let mut blocks_shortened = blocks[5..].iter().cloned().collect();
    let mut qc_shortened = quorum_certs[5..].iter().cloned().collect();

    // Subset of blocks is inputted to `find_root()` so that LI(S) is disconnected from LI(C).
    // `find_root()` returns an Error.
    assert!(RecoveryData::find_root(&mut blocks_shortened, &mut qc_shortened, &storage_ledger).is_err());
}
