...
impl ChainManager {
    ...
    async fn execut_and_commit_block(
        ...
        let compute_res = state_computer
            .compute(block.parent_id(), block.id(), &payload)
            .await
        ...
        if payload.len() > 0 {
            if let Err(e) = txn_manager
                .commit_txns(&payload, &compute_res, block.timestamp_usecs())
                .await
            {
        ...
        if let Err(e) = state_computer
            .commit_with_id(block.id(), block.quorum_cert().ledger_info().clone())
            .await
        {
