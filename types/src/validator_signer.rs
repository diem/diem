...
impl ValidatorSigner {
    pub fn new(author: AccountAddress, private_key: ed25519::SigningKey) -> Self {
        ValidatorSigner {
            author,
            private_key,
        }
...
impl ValidatorSigner {
    ...
    pub fn random(opt_rng_seed: impl for<'a> Into<Option<[u8; 32]>>) -> Self {
        ...
        Self::new(
            ...
            ed25519::SigningKey::generate(&mut rng),
        )
    }
...
pub mod proptests {
    ...
    pub fn arb_signing_key() -> impl Strategy<Value = ed25519::SigningKey> {
        prop_oneof![
            // The no_shrink here reflects that particular keypair choices out
            // of random options are irrelevant.
            LazyJust::new(|| ed25519::SigningKey::generate_for_testing()),
            LazyJust::new(|| ed25519::SigningKey::genesis()),
        ]
    }

    pub fn signer_strategy(
        signing_key_strategy: impl Strategy<Value = ed25519::SigningKey>,
    ...
    pub fn arb_signer() -> impl Strategy<Value = ValidatorSigner> {
        prop_oneof![
            rand_signer(),
            LazyJust::new(|| {
                let genesis_key = ed25519::SigningKey::genesis();
    ...
    fn select_keypair(
        keys: Vec<ed25519::SigningKey>,
    ) -> impl Strategy<Value = ed25519::SigningKey> {
        sample::select(keys)
    }
