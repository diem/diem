...
---
This document will guide you through executing your first transaction on the Diem Blockchain's <<glossary:testnet>>. Before you proceed, we recommend that you read the following to familiarize yourself with the key concepts:
...
The sequence number for an account indicates the number of transactions that have been sent from that account. It is incremented by one every time a transaction sent from that account is executed and stored in the Diem Blockchain. To learn more, refer to <<glossary:sequence number>>.
