...
fn test_get_set_non_existent(storage: &mut dyn Storage) {
    let crypto_value = Value::Ed25519PrivateKey(ed25519::SigningKey::generate_for_testing());
...
fn test_create_get_set_unwrap(storage: &mut dyn Storage) {
    let crypto_private_1 = ed25519::SigningKey::generate_for_testing();
    let crypto_private_2 = ed25519::SigningKey::generate_for_testing();
    let u64_1 = 10;
    let u64_2 = 647;
...
fn test_create_and_get_non_existent_version(storage: &mut dyn Storage) {
    ...
    let non_existent_public_key = ed25519::SigningKey::generate_for_testing().public_key();
    assert!(
